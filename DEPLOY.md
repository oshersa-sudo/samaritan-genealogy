...
## שלב 2 — הגדרת סיסמת-המנהל (סוד)
ב-**Worker** של הפרויקט → **Settings** → **Variables and Secrets** →
**Add** → סוג **Secret** → שם בדיוק `ADMIN_PASSWORD` → ערך = הסיסמה שתבחרו → **Save**.
(הסיסמה נשמרת מוצפנת בצד-השרת בלבד; לעולם לא בקוד ולא ב-git.)
