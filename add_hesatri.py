...
# --- page_88: תמים בן עבד-חנונה (#135) וילדיו; אברהם #140 וילדיו (חוסרים מהמיפקד) ---
def has(pid): return any(p['id'] == pid for f in (fam for h in master['houses'] for fam in h['families']) for p in f['persons'])
# attach תמים as son of census #135 (עבד-חנונה)
if has('135'):
    g135 = next(p for f in (fam for h in master['houses'] for fam in h['families']) for p in f['persons'] if p['id'] == '135')
    g135.setdefault('children_ids', [])
    if 'הס-תמים' not in g135['children_ids']:
        g135['children_ids'].append('הס-תמים')
P('הס-תמים', 'תמים (פֿאמל)', 'M', '135', '1870–1949',
  ...
  children=['הס-פוזי', 'הס-סברי', 'הס-חיכמת'])
P('הס-פוזי', 'פוֹזי', 'M', 'הס-תמים', '1872–1923',
  'בן תמים (מאשתו לאה). עזר לאביו בכתיבת בקשות; חלה בנמק ברגלו ומת בניתוח קטיעה.')
P('הס-סברי', 'סברי', 'M', 'הס-תמים', '1900–',
  'בן תמים. עסק בנגרות כל ימיו; העתיק כתב-יד או שניים.')
P('הס-חיכמת', 'ישמעאל (חיכמת)', 'M', 'הס-תמים', '1904–1985',
  'בן תמים. נקרא על שם אביו המת (נולד 4 ימים אחרי פטירתו). חייט; ידע על-פה בראשית ושמות; משירה; העתיק כתבי-יד רבים.')
# אברהם #140 -> עבד-אלה, סלים
if has('140'):
    g140 = next(p for f in (fam for h in master['houses'] for fam in h['families']) for p in f['persons'] if p['id'] == '140')
    g140['children_ids'] = ['הס-עבדאלה140', 'הס-סלים140']
P('הס-עבדאלה140', 'עבד-אלה (עבד אללטיף)', 'M', '140', '1903–',
